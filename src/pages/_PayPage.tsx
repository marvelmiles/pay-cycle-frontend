...
const cardSchema = z.object({
  cardNumber: z
    ...
    .refine((v) => /^\d{16}$/.test(v), "Enter a valid 16-digit card number"),
...
export const PayPage: React.FC = () => {
  ...
  const [iswitchCfg, setIswitchCfg] = useState<InterswitchConfig | null>(null);
  ...
  const card = useForm<CardForm>({ resolver: zodResolver(cardSchema) });
...
// CALLBACK PAGE
// ─────────────────────────────────────────────
export const PayCallbackPage: React.FC = () => {
  const { slug } = useParams<{ slug: string }>();
  ...
  useEffect(() => {
    ...
    payService
      ...
      .then((res) => {
        ...
        setResult(d.status === "successful" ? "success" : "failed");
      })
        ...
        {result === "success" && (
          <>
            ...
            <div className="mt-6 pt-6 border-t border-gray-100 flex items-center justify-center gap-2 text-xs text-gray-400">
              ...
              Powered by BillFlow · Secured by Interswitch
            </div>
          </>
        )}
        {result === "failed" && (
          <>
            ...
            {slug && (
              <a
                href={`/pay/${slug}`}
                className="mt-6 inline-flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-700 text-white font-medium px-5 py-2.5 rounded-lg text-sm transition-colors"
              >
                Try again
              </a>
            )}
